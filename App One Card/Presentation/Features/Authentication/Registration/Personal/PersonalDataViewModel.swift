                ...
                self.delegate?.hideLoader {
                    if response.validExpiration == "1" {
                        if response.exists == "1" {
                            ...
                            self.verificationRouter.navigateToVerification(email: email, number: cellphone, documentType: self.documentType, documentNumber: self.documentNumber, companyRUC: self.companyRUC, navTitle: "REGISTRO DE USUARIO DIGITAL", stepDescription: "Paso 3 de 4") { [weak self] otpId in
